...
namespace lab07 {
    internal partial class Queue<T> where T : class {
        ...
        public T Remove() {
            if (Empty) {
                throw new CollectionError("Can't execute 'Remove' method on empty Queue");
            }
        ...
        public static Queue<T> operator --(Queue<T> queue) {
            try {
                ...
                throw new CollectionError($"Error while excecuting operator '--': {err.Message}");
            }
        }
